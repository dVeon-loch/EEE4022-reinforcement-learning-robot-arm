...
class A2C:
    ...
    def actor_net(self, state, nb_actions, name, reuse=False, training=True, use_layer_norm=True):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            x = tf.compat.v1.layers.Dense(130)(state)
            if use_layer_norm:
                x = tf.contrib.layers.layer_norm(x)
            x = tf.nn.relu(x)
            x = tf.compat.v1.layers.Dense(100)(x)
            if use_layer_norm:
                x = tf.contrib.layers.layer_norm(x)
            x = tf.nn.relu(x)
            x = tf.compat.v1.layers.Dense(80)(x)
            if use_layer_norm:
                x = tf.contrib.layers.layer_norm(x)
            x = tf.nn.relu(x)
            actions = tf.compat.v1.layers.Dense(nb_actions,
                                      activation=tf.tanh,
    ...
    def critic_net(self, state, action, name, reuse=False, training=True, use_layer_norm=True):
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            if use_layer_norm:
                x = tf.contrib.layers.layer_norm(x)
            x = tf.nn.relu(x)
            q = tf.compat.v1.layers.Dense(1,kernel_initializer=tf.random_uniform_initializer(minval=-3e-3, maxval=3e-3))(x)
            return q
...
class DDPG:
    ...
    def initialize(self):
        ...
        actor_var = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='actor')
        actor_target_var = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='target_actor')
        critic_var = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='critic')
        critic_target_var = tf.compat.v1.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='target_critic')
        target_init_ops = []
        soft_update_ops = []
